...
class BatchCRF(nn.Module):
    def __init__(self, tag_to_ix, use_cuda=True, average_batch=True):
        ...
        # self.transitions = nn.Parameter(torch.randn(self.tagset_size, self.tagset_size))
        # # # These statements enforce the constraint that we never transfer
    ...
    def _neg_log_likelihood_loss(self, feats, mask, tags):
        batch_size = feats.size(0)
        mask = mask.bool()
